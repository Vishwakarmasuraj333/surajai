...
import { Eye, EyeOff, Lock, Mail, ArrowRight, AlertCircle, ShieldCheck, Sparkles, Layers } from 'lucide-react';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    // High-tech 2-second delayed pop-up entrance so user inspects AI Architecture diagram first!
  ...
  return (
    <div className="min-h-screen bg-[#04040a] text-gray-100 flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <header className="p-6 md:p-8 relative z-20 flex items-center justify-between">
        ...
        {/* Floating Architecture Toggle & System Online Badge */}
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => setShowForm(!showForm)}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/40 text-cyan-300 hover:text-white hover:bg-cyan-500/20 text-xs font-semibold backdrop-blur-md shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all cursor-pointer"
          >
            <Layers className="w-3.5 h-3.5 text-cyan-400" />
            <span>{showForm ? 'View Full Architecture' : 'Sign In Form'}</span>
          </button>

          <div className="hidden sm:flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[11px] font-mono backdrop-blur-md shadow-[0_0_15px_rgba(16,185,129,0.2)]">
      ...
      {/* Floating Bottom Hint when Form is Hidden */}
      {!showForm && (
        <div
          onClick={() => setShowForm(true)}
          className="fixed bottom-12 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3 px-6 py-3 rounded-full bg-[#090a16]/90 border border-cyan-400/50 text-xs font-mono text-cyan-300 backdrop-blur-xl shadow-[0_0_35px_rgba(6,182,212,0.4)] animate-pulse cursor-pointer hover:scale-105 transition-transform"
        >
          <Sparkles className="w-4 h-4 text-cyan-400 animate-spin" />
          <span>Viewing SurajAI Architecture Nodes • Tap to Open Sign In</span>
        </div>
      )}
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 relative z-20 my-auto">
        {showForm && (
          <div className="w-full max-w-[420px] cinematic-glass-card rounded-3xl p-7 sm:p-9 space-y-5 relative overflow-hidden animate-in zoom-in-95 fade-in duration-500 shadow-[0_0_60px_rgba(6,182,212,0.25)]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full light-sweep-btn inline-flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-600 hover:from-cyan-300 hover:via-blue-400 hover:to-indigo-500 text-white font-bold py-3.5 px-5 rounded-xl shadow-[0_0_30px_rgba(6,182,212,0.4)] hover:shadow-[0_0_40px_rgba(168,85,247,0.55)] hover:scale-[1.02] active:scale-[0.98] transition-all text-sm disabled:opacity-50 mt-2 cursor-pointer"
              >
                {loading ? (
